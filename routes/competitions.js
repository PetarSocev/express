...
router.get("/logs/:id", adminRequired, function (req, res, next) {
    res.render("competitions/logs");
});

router.post("/logs", authRequired, function (req, res, next) {


    const stmt = db.prepare("INSERT INTO logs (log) VALUES (?);");
    const updateResult = stmt.run(req.body.log);

    if (updateResult.changes && updateResult.changes === 1) {
        res.render("competitions/logs", { result: { success: true } });
    } else {
        res.render("competitions/logs", { result: { database_error: true } });
    }
});

router.get("/logsview/:id", function (req, res, next) {
    ...
    res.render("competitions/logsview", { result: { items: result}});
});
